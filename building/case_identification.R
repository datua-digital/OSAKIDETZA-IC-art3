...
#' @param early_death_patients_days: (int) days to consider patients die too quickly
#' @param drugs: (character) Global variable drugs
#'
#' @return df: baseJoinModel with three new boolean variables:
...
#' Identify patients that had not suffered IC before being entered
#'
#' @param df_: (data.frame) baseJoinModel
...
#' get active principle of some families
#'
#' @param drugs: (character) Global variable drugs
#'
#' @return
...
identify_TT_denovopatients <- function(df_, presc, ing_pri, drugs){
  ...
  # Para Eduardo, Cuál de los dos usar?????
  #prescripcion antes del alta (solo 545 pacientes no tienen prescripcion antes del alta)
  presc_ing2 <- presc_ing %>% filter(fecha_inicio<falta_ing1)
  
  #prescripcion antes del ingreso (solo 583 pacientes no tienen prescripcion antes del alta)
