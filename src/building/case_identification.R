...
case_identification <- function(df, early_death_patients_days, drugs) {
  ...
  df <- identify_tt_denovopatients(df, presc = cohorteicc2::presc,
                                   ing_pri = cohorteicc2::ingresos_pri, drugs = drugs)
...
identify_early_death_patients <- function(df_, early_death_patients_days, outcome) {
  fallecidos <- outcome %>% 
    dplyr::filter(dias_hasta_muerte <= early_death_patients_days)
  df_ <- df_ %>% 
    dplyr::mutate(early_death_patient = dplyr::if_else(id %in% fallecidos$id, TRUE, FALSE))
...
identify_ic_denovopatients <- function(df_, ap, ing) {
  ...
  ap <- ap %>%
    dplyr::filter(fing_ing1 > finicio) %>%
    dplyr::filter(grepl("^39891|^40211|^40291|^40401|^40403|^40411|^40413|^40491|^40493|^428", cie))
  ...
  ing <- ing %>%
    dplyr::filter(fing_ing1 > fing) %>%
    dplyr::filter(grepl("^39891|^40211|^40291|^40401|^40403|^40411|^40413|^40491|^40493|^428|^I0981|^I110|^I130|^I132|^I50", cie))
  df_ <- df_ %>%
...
identify_tt_denovopatients <- function(df_, presc, ing_pri, drugs) {
  ...
  ing_pri <- ing_pri %>%
    ...
    dplyr::filter(id %in% df_$id) %>%
    dplyr::select(id, fing_ing1, falta_ing1)
  presc_ing <- presc %>%
  ...
  df_ <- df_ %>%
    dplyr::mutate(denovo_tt_paciente_fing = dplyr::if_else((!id %in% presc_ing3$id), TRUE, FALSE),
